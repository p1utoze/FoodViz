...
def get_food_code() -> list:
    ss = _sess_state["units_df"].loc[
        (_sess_state["units_df"]["name"].isin(_sess_state["nutrient"])), "code"
    ]
    return ss[~ss.str.endswith("_e")].tolist()


@st.cache_data(ttl=600)
def get_nutrient_column(
    table: str, return_columns: str = "*", as_df: bool = True
):
    return_columns = f"name, {return_columns}"
    response = (
        _sess_state["_conn"].table(table).select(return_columns).execute()
    )
    if as_df:
...
st.multiselect(
    "Select Nutrient",
    _sess_state["units_df"]
    .loc[_sess_state["units_df"]["type"].isin(["mass", "energy"]), "name"]
    .unique(),
    key="nutrient",
    max_selections=10,
)
...
if "nutrient" in _sess_state and _sess_state["nutrient"]:
    ...
    units = (
        _sess_state["units_df"]
        .loc[_sess_state["units_df"]["code"].isin(cols), "unit"]
        .tolist()
    )
    factors = (
        _sess_state["units_df"]
        .loc[_sess_state["units_df"]["code"].isin(cols), "factor"]
        .tolist()
    )
    ...
    # print(config)
    st.dataframe(
        response, column_config=config, use_container_width=True, height=500
    )
