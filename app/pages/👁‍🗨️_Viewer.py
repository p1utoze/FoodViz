...
st.markdown("> This page provides details of the individual food items provided by Indian Food Composition Tables (IFCT) database. "
         "You can view the details of the food items and their composition in the database.")
...
with tab1:
    with st.container(height=100):
        ...
        grp_name = col1.selectbox("Choose a food group: ", _sess_state["fg"].keys(), index=None,
                                  placeholder="Select a food group", on_change=tab1_box1_callback)
        if grp_name:
...
with tab2:
    ...
    if _sess_state["selected_code"]:
        food_item_df = query_with_filter_eq(
            ...
            table=DB_TABLE_NAME,
            column=DB_FOOD_CODE_HEADER,
            value=_sess_state["selected_code"]
        )
